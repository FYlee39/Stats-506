...
generate_data <- function(n, x_poly) {
  ...
  data_set <- data.frame(x_1=x_1, x_2=x_2, 
                         y=beta_0 + beta_1 * x_1 + beta_2 * x_2 + noisy)
  return(data_set)
}
...
for (n in 1: length(sample_size)){
  for (q in 1: length(sample_degree)){
    ...
    for (i in 1: num_simulations){
      ...
      normal_CI <- confint(model, 'x_2', level=0.95)
      normal_covrage_condition[i] <- (beta_2 >= normal_CI[1] & beta_2 <= normal_CI[2])
      ...
      lower_bd <- model$coefficients[3] - t_criticcal * se_HC2[3]
      upper_bd <- model$coefficients[3] + t_criticcal * se_HC2[3]
      robust_CI <- cbind(lower_bd, upper_bd)
      robust_covrage_condition[i] <- (beta_2 >= robust_CI[1] & beta_2 <= robust_CI[2])
    }
